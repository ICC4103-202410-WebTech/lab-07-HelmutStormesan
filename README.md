...
```ruby
### Code here
johns_posts = Post.joins(:user).where(users: {name: "John Doe"})
...
```ruby
### Code here
tags_for_post1 = Tag.joins(:posts).where(posts: {title: "Post 1"})
...
```ruby
### Code hereusers_with_tag1 = User.joins(posts: :tags).where(tags: {name: "Tag 1"})
